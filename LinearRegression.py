...
for key, group in df.groupby(["Material Type", "Material Sub Type", "Material Description", "Plant", "Supplier"], sort=False):
    group = group.sort_values("Final Price Month")
    group[price_columns] = group[price_columns].fillna(0)

    data = group[["Final Price Month", *price_columns]].values.tolist()
