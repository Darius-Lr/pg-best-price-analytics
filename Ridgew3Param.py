...
OUTPUT_FILE = "outputRidge3Param.txt"
ALPHA = 1.0
GROUP_COLUMNS = [
    "Material Type",
    "Material Sub Type",
...
output_lines = [f"Ridge alpha used: {ALPHA}"]
errors = []
...
for key, group in df.groupby(GROUP_COLUMNS, sort=False):
    ...
    if len(group) < 2:
        output_lines.append(
            "Nu exista suficiente date pentru modelul Ridge cu 3 parametri. Sunt necesare cel putin 2 luni."
        )
        output_lines.append("")
        continue
    ...
    output_lines.append(
        format_comparison("Sklearn Ridge (3 params)", predicted_price, actual_final_price)
    )
    output_lines.append(
...
if errors:
    output_lines.append(
        f"Overall MAE for Sklearn Ridge (3 params): {format_number(sum(errors) / len(errors))}"
    )
