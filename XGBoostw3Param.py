...
OUTPUT_FILE = "outputXGBoost3Param.txt"
N_ESTIMATORS = 200
LEARNING_RATE = 0.05
MAX_DEPTH = 3
SUBSAMPLE = 1.0
...
output_lines = [
    f"XGBoost n_estimators used: {N_ESTIMATORS}",
    f"XGBoost learning_rate used: {LEARNING_RATE}",
    f"XGBoost max_depth used: {MAX_DEPTH}",
    f"XGBoost subsample used: {SUBSAMPLE}",
    f"XGBoost colsample_bytree used: {COLSAMPLE_BYTREE}",
    f"XGBoost random_state used: {RANDOM_STATE}",
]
errors = []
...
for key, group in df.groupby(GROUP_COLUMNS, sort=False):
    ...
    if len(group) < 2:
        output_lines.append(
            "Nu exista suficiente date pentru modelul XGBoost cu 3 parametri. Sunt necesare cel putin 2 luni."
        )
        output_lines.append("")
        continue
    ...
    output_lines.append(
        format_comparison("XGBoost Regressor (3 params)", predicted_price, actual_final_price)
    )
    output_lines.append(
        "Feature importances: " + format_feature_importances(model, FEATURE_COLUMNS)
    )
    output_lines.append("")

if errors:
    output_lines.append(
        f"Overall MAE for XGBoost Regressor (3 params): {format_number(sum(errors) / len(errors))}"
    )
